...
def Mariage_stable(capacity,p_E,p_S,debug=False):
    ...
    pref_E = [list(e) for e in p_E]
    pref_S = [list(s) for s in p_S]
    ...
    print(pref_E)
    currently_choosen = [not(e) for e in pref_E] #vaut vrai si les étudiants sont choisis ou si ils ont épuisé leurs préférences
    current_student_in_school = [[] for i in pref_S]#On veut garder chaque sous liste triée par préférence de l'école
    while (currently_choosen != [True]*len(pref_E)): #Tant que tout le monde n'est pas choisi et que les préférences ne sont pas épuisées
        if debug:
            print("les étudiants arrivé au bout de leur préférences ou chosis sont les suivants : "+str(currently_choosen))
        demandes = [ [] for i in range(len(pref_S))] #Pour chaque école la liste des étudiants qui l'ont demandée
        ...
        for i in range(len(pref_S)):
            for j in range(len(demandes[i])):
                ...
                if(len(current_student_in_school[i]) < capacity[i]):
                    ...
                    if(etu in pref_S[i] and current_student_in_school[i][-1] in pref_S and pref_S[i].index(current_student_in_school[i][-1]) > pref_S[i].index(etu)):
...
def generate_instance(capacity,etu_size,random_pref_size=False):
