...
def Mariage_stable(capacity,pref_E,pref_S):
    ...
    if(len(pref_E) < max([max(p) for p in pref_S])):
        print("Il se peut qu'il y ait une erreur dans l'appel a la fonction car certains étudiants n'ont pas de liste de préférence, la liste [] leur sera attribué")
    if(len(pref_E) > max([max(p) for p in pref_S])):
        ...
        for i in range(len(pref_S)):
            for j in range(len(demandes[i])):
                ...
                if(len(current_student_in_school[i]) < capacity[i]):
                    ...
                    if(pref_S[i].index(current_student_in_school[i][-1]) > pref_S[i].index(etu)):
...
def generate_instance(capacity,etu_size):
    ...
    for s in range(school_size):
        pref_size = randint(1,int(etu_size*1.3))
        pref_S.append([])
        for e in range(pref_size):
            r = randint(0,etu_size)
            while r in pref_S[s]:
                r = randint(0,etu_size)
            pref_S[s].append(r)
    pref_E = []
    for e in range(etu_size):
        pref_size = randint(1,int(school_size*1.3))
        pref_E.append([])
        for s in range(pref_size):
            r = randint(0,school_size)
            while r in pref_E[e]:
                r = randint(0,school_size)
            pref_E[e].append(r)
